...
\begin{definitie}{Het pompend lemma voor reguliere talen}
  Voor een reguliere taal $L$ bestaat een pomplengte $d$ zodanig dat als $s \in L$ en $|s| \geq d$, dan bestaat er minstens \'e\'en verdeling van $s$ in stukken $x$, $y$ en $z$ zodanig dat $s = xyz$ en
  \begin{enumerate}
...
\begin{definitie}{Contextvrije grammatica}
  ...
  \item $R$ een eindige verzameling regels (producties) is die bestaan uit een koppel van een variabele en een string $s \in \Sigma^*_\epsilon$, gescheiden door $\rightarrow$
...
\begin{definitie}{Taal $L_{CFG}$ bepaald door een CFG}
  ...
  \begin{equation*}
  L_{CFG} = \{s \in \Sigma^*|S \Rightarrow^* s\}
  \end{equation*}
\end{definitie}
...
\begin{definitie}{Chromsky Normaal Vorm}
  Elke CFG heeft een Chromsky Normaal Vorm (CNF) die dezelfde taal genereert en waarbij de producties een beperkt aantal vormen kan aannemen:
  \begin{itemize}
...
Om een meest-linkse afleiding te bekomen moeten we in algoritme \ref{alg:cfgpda} de symbolen van een string $\gamma \in \Sigma^*_\epsilon$ in omgekeerde volgorde op de stapel plaatsen.
