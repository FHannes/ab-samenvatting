...
\begin{definitie}{De verzameling $L_\Sigma$ van alle talen over $\Sigma$}
  $L_\Sigma = \powerset(\Sigma^*)$ is een verzameling met alle talen die gevormd kunnen worden uit het alfabet $\Sigma$. Elke taal over $\Sigma$ is een element van $\powerset(\Sigma^*)$.
\end{definitie}
...
\begin{definitie}{De verzameling $RegLan$ van de reguliere talen}
  ...
  \\ $x \in RegLan \en y \in x \Rightarrow y \in \Sigma^*$
\end{definitie}
...
\begin{definitie}{Een niet-deterministische eindige toestandsautomaat}
  ...
  \item \bm{$\delta : Q \times \Sigma_\epsilon \rightarrow \powerset(Q)$}\\$\delta$ is de overgansfunctie
...
Elke NFA beschijft een reguliere taal en elke reguliere taal kan beschreven worden door een NFA ($L_E = L_{NFA_E}$). Figuur \ref{fig:regexnfa} toont de overgangen van de elemetaire reguliere expressies naar de bijhorende NFA's.
...
\begin{definitie}{Een gegeneraliseerde NFA}
  ...
  \item Het label van elke boog is een reguliere epxressie.
  \end{itemize}
\end{definitie}
...
\begin{algoritme}{Transformatie NFA naar DFA}
  ...
  \begin{itemize}
  \item \bm{$eb: Q_n \rightarrow \powerset(Q_n)$}\\ $eb(q) = \{x|x \in \{q\} \cup \delta_n(q, \epsilon)\}$ met $q \in Q_n$: De afbeelding die een toestand $q \in Q_n$ afbeeldt op alle epsilon-bereikbare toestanden.
...
\begin{definitie}{De overgangsfunctie $\delta^*$ van een DFA voor strings}
  \label{def:deltaster}
  \bm{$\delta^*: (\powerset(Q) \times \Sigma^*) \rightarrow \powerset(Q)$}\\
  ...
  \begin{equation*}
    \delta^*(q, s) = \begin{cases}
      q, & \text{als}\ s=\epsilon \\
      \delta^*(\delta(q, a), w) & \text{als}\ s=aw\ \text{voor}\ a \in \Sigma, w \in \Sigma^*
    \end{cases}
  \end{equation*}
\end{definitie}

\begin{bewijs}
  We bewijzen dat $\delta^*(q, wa) = \delta(\delta^*(q, w), a)$ voor $a \in \Sigma, w \in \Sigma^*_\epsilon$.\\[.2cm]
  ...
  \item Stel $w \neq \epsilon$ en $w = bw'$ met $b \in \Sigma, w' \in \Sigma^*_\epsilon$, dan geldt:\\$\delta^*(q, wa) = \delta^*(q, bw'a) = \delta^*(\delta(q, b), w'a) = \delta(\delta^*(\delta(q, b), w'), a) = \delta(\delta^*(q, w), a)$
  \end{itemize}
\end{bewijs}
...
\subsubsection{Myhill-Nerode relaties op $\Sigma^*$}
...
\begin{definitie}{$\partition_1$ is groffer dan $\partition_2$}
  Als een partie $\partition_1$ groffer is dan een $\partition_2$, dan zit elk element van $\partition_2$ vervat in een element van $\partition_1$. $\{\partition_2$, $\partition_1\}$ vormt een tralie waar geldt dat $\partition_2 \leq \partition_1$.
\end{definitie}

\begin{definitie}{Equivalentierelatie $\sim_L$ over $\Sigma^*$}
  Gegeven een taal $L$ over $\Sigma$, is het mogelijk om $\Sigma^*$ op te delen in de partitie $\{L,\overline{L}\}$. De equivalentierelatie $\sim_L$ over $\Sigma^*$ beschouwt twee elementen van $\Sigma^*$ gelijkwaardig indien ze beiden tot dezelfde deelverzameling van die partitie behoren.
  \begin{equation*}
  \forall x, y \in \Sigma^*: x \sim_L y \Leftrightarrow x,y \in L \of x,y \notin L
  \end{equation*}
\end{definitie}

\begin{definitie}{De deelverzameling $reach(q)$ van $\Sigma^*$ voor een DFA}
  De deelverzameling $reach(q)$ van $\Sigma^*$ voor een DFA ($Q$, $\Sigma$, $\delta$, $q_s$, $F$) bevat alle strings die je vanaf de begintoestand $q_s$ in de toestand $q \in Q$ brengen.
  \begin{equation*}
  reach(q) = \{s \in \Sigma^*|\delta^*(q_s,s)=q\}
  \end{equation*}
\end{definitie}

\begin{definitie}{Equivalentierelatie $\sim_{DFA}$ over $\Sigma^*$}
  Gegeven een $DFA$ met $L_{DFA}$ een taal over $\Sigma$, is het mogelijk om $\Sigma^*$ op te delen in de een partitie $\{reach(q)|q \in Q\}$ zodanig dat
  \begin{itemize}
  \item de partitie eindig is.
  \item $s \in \Sigma^*$ wordt aanvaard door de $DFA$ alss $s \in reach(q)$ voor $q \in F$.
  \item als $s \in reach(q)$ dan $\forall a \in \Sigma : sa \in reach(\delta(q, a))$.
  \end{itemize}
  Of in symbolen:
  \begin{equation*}
  \forall x, y \in \Sigma^*: x \sim_{DFA} y \Leftrightarrow \delta^*(q_s, x) = \delta^*(q_s, y)
  \end{equation*}
\end{definitie}

De equivalentierelatie $\sim_{DFA}$ is een $MN(L_{DFA})$-relatie op $\Sigma^*$.

\begin{definitie}{Een Myhill-Nerode relatie voor een taal $L$ over $\Sigma^*$}
  \label{def:mnl}
...
\subsubsection{Pompend lemma voor reguliere talen}
